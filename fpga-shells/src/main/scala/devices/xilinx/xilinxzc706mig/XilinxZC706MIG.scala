...
import sifive.fpgashells.ip.xilinx.zc706mig.{ZC706MIGIOClocksReset, ZC706MIGIODDR, zc706mig}

case class XilinxZC706MIGParams(
  address : Seq[AddressSet]
)

class XilinxZC706MIGPads(depth : BigInt) extends ZC706MIGIODDR(depth) {
  def this(c : XilinxZC706MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxZC706MIGIO(depth : BigInt) extends ZC706MIGIODDR(depth) with ZC706MIGIOClocksReset

class XilinxZC706MIGIsland(c : XilinxZC706MIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxZC706MIGIO(depth)
    })
    ...
    val blackbox = Module(new zc706mig(depth))
...
class XilinxZC706MIG(c : XilinxZC706MIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxZC706MIGIsland(c, crossing))
