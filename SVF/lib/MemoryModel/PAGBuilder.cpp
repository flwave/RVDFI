...
void PAGBuilder::initalNode() {
    ...
    int nval = 0;
    int id = 1;
    while (nval < symTable->idToValMap().size()) {
        while (symTable->idToValMap().find(id) == symTable->idToValMap().end()) { ++id; }
        const Value *val = symTable->idToValMap()[id];
        DBOUT(DDFI, outs() << id << "\t");
        if (symTable->objSyms().find(val) != symTable->objSyms().end()) {
            SymID oid = symTable->getObjSym(val);
            if (symTable->getObj(oid)->isFunction()) {
                Function *fun = (Function *) val;
                DBOUT(DDFI, outs() << fun->getName() << "(");
