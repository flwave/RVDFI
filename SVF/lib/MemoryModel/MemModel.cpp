...
/*
 * Get the type name of the memory object
 */
string MemObj::getMemTypeStr() {
    string memType;
    if (isFunction()) memType += "Func";
    if (isGlobalObj()) memType += "Glob";
    if (isStaticObj()) memType += "Static";
    if (isStack()) memType += "Stack";
    if (isHeap()) memType += "Heap";
    if (isStruct()) memType += "Struct";
    if (isArray()) memType += "Array";
    if (isVarStruct()) memType += "VarStruct";
    if (isVarArray()) memType += "VarStruct";
    if (isConstStruct()) memType += "ConstStruct";
    if (isConstArray()) memType += "ConstArray";
    if (isConstant()) memType += "Const";
    if (hasPtrObj()) memType += "HasPtr";
    return memType;
}
...
void SymbolTableInfo::collectVal(const llvm::Value *val) {
    ...
    if (iter == valSymMap.end()) {
        ...
        symValMap.insert(std::make_pair(totalSymNum, val));
        DBOUT(DMemModel,
...
void SymbolTableInfo::collectObj(const llvm::Value *val) {
    assert(valSymMap.find(val) != valSymMap.end());
