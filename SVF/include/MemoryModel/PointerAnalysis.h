...
public:
    ...
    //@{
    typedef std::map<NodeID, DefSet> IDToDefSetMap;
    //@}
...
public:
    ...
    /// Reaching definition set
    //@{
    virtual void genDefSet(NodeID set, NodeID def);
    virtual void genDefSet(NodeID set, const DefSet &defs);
    virtual void genDefSet();
    virtual std::string getDefSetString(SymID sym);
    virtual void dumpDefSet();
    virtual void logFunctionCall();
    inline DefSet &getDefSet(NodeID node) {
        assert(defSet.find(node) != defSet.end());
        return defSet[node];
    }
    inline IDToDefSetMap &getDefSetMap() { return defSet; }
    virtual void genDefSet(NodeID def, const llvm::Value *val);
    virtual void genReachDefSet(NodeID tgt, const llvm::Value *val);
    // Instrument IR for DFI
