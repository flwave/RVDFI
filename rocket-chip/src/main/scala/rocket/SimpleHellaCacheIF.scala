 ...
 * completed.
 */
class DFISimpleHellaCacheIFReplayQueue(depth: Int)
    (implicit val p: Parameters) extends Module
    with HasL1HellaCacheParameters {
  val io = new Bundle {
    val req = Decoupled(new HellaCacheReq).flip
  ...
  //flang
  val replay_complete = (io.resp.valid && replaying && io.resp.bits.tag === next_replay_req.tag) || (timeout === Bits(maxtimeout))
  
  when(replay_complete || timeout>=Bits(maxtimeout) || !nackq.io.deq.valid || Bool(true)){
    when(timeout>=Bits(maxtimeout)){
      printf("WARNING: replay timeout\n")
    }
    timeout := Bits(0)
  }.otherwise{
...
// exposes a sane decoupled request interface
class DFISimpleHellaCacheIF(implicit p: Parameters) extends Module
{
  val io = new Bundle {
    val requestor = new HellaCacheIO().flip
    val cache = new HellaCacheIO
  ...
  val replayq = Module(new DFISimpleHellaCacheIFReplayQueue(2))
  val req_arb = Module(new Arbiter(new HellaCacheReq, 2))
